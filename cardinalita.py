...
def cardi():
    ...
    try:
        # Dati d'input
        path= config.get('data-input','gdb')
        ...
        arcpy.CreateFeatureclass_management(path,out_transetti,"POLYLINE","","","",transetti)
        arcpy.CreateFeatureclass_management(path, transetti_sort2, "POLYLINE", "", "", "", transetti)
        arcpy.CreateFeatureclass_management(path, tran_card1, "POLYLINE", "", "", "", transetti)


        pozzetti_V2 = arcpy.analysis.SpatialJoin("Pozzetti","Transetti",r"C:\MUIF\SINFI\NUOVA SOLUZIONE_CAVIDOTTI\Prj_gis\Cavidotti\Cavidotti.gdb\Pozzetti_V2",
        ...
        arcpy.CopyFeatures_management("Vertici_Nuovi", "C:\MUIF\SINFI\Versionamento Progetto\SubTest_Piemonte\Teas_Cardinalita.gdb\pozzetti_new")
        ...
        with arcpy.da.SearchCursor(r"C:\MUIF\SINFI\Versionamento Progetto\SubTest_Piemonte\Teas_Cardinalita.gdb\pozzetti_new", ['Shape@']) as cursor1:
            conto3 = 0
            for row1 in cursor1:
                # usa la geometria per la selezione
                pozz1 = row1[0]
        ...
        arcpy.management.DeleteIdentical("tran_card1", "Shape", None, 0)
        tr_da_eliminare = arcpy.management.SelectLayerByLocation(
            in_layer="Transetti",
            overlap_type="BOUNDARY_TOUCHES",
            select_features="tran_card1",
